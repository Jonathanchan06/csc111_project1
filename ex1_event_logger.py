...
class EventList:
    ...
    def add_event(self, event: Event, command: Optional[str] = None) -> None:
        ...
        if self.first is None:
            ...
            self.last.next_command = command  # The command that leads from old last to the new event
            self.last.next = event
            event.prev = self.last
            event.next = None
            event.next_command = None  # The new last event doesn't yet have a next command
            self.last = event
    ...
    def remove_last_event(self) -> None:
        ...
        if self.is_empty():
            ...
            # Find the event before the last one
